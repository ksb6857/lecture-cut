...
- 전사는 로컬 위스퍼(무료)만으로 끝까지 된다. ElevenLabs API 키가 있으면 Scribe(유료)를 쓸 수 있다.
  한 강의로 재 보니 Scribe 가 낱말 시각(시작 오차 90%: 0.51초 → 0.09초)과 용어 표기에서 앞섰다.
  Scribe 가 지운 되풀이는 다시 전사해 메운다(`fill_uncovered`). 두 전사를 같은 녹음으로 비교하는 도구도 있다
...
ElevenLabs 는 없어도 된다. 키가 있으면 Scribe 전사를 고를 수 있다. 키는 환경변수 `ELEVENLABS_API_KEY` 나
저장소 밖 파일(`~/.elevenlabs.key`, 또는 `ELEVENLABS_KEY_FILE` 이 가리키는 곳)에 둔다. 저장소 안에 두거나 커밋하지 않는다.
