...
사용: python render_preview.py <소재.mp4> <keep.json> <출력.mp4>
        [--boxes 박스.json] [--transitions 전환.json] [--levels 세기.npy --offset 초] [--until 편집초] [--fps 30]
...
def render(src, keep, out, boxes=(), cuts=(), db=None, offset=0.0, until=None, fps=30, quiet=-62.0):
    blocks, edges, e = [], [], 0.0
    for a, b in keep:
    ...
    with open(raw, "rb") as fi, open(cut, "wb") as fo:
        for k, (a, b, f0, f1) in enumerate(blocks):
            ...
            fo.write(buf)
    subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", str(src), "-f", "s16le", "-ar", str(SR), "-ac", "2",
                    "-i", str(cut), "-/vf", str(script), "-map", "0:v", "-map", "1:a", "-c:v", "libx264",
                    "-preset", "veryfast", "-crf", "24", "-r", str(fps), "-c:a", "aac", "-b:a", "128k",
                    "-frames:v", str(nframes), "-movflags", "+faststart", str(out)], check=True)
    for f in (raw, cut):
